...
impl AgentManager {
    ...
    fn build_openai_supervisor(&self, config: &AgentConfig, evidence_pool: sqlx::SqlitePool, app: tauri::AppHandle, evidence_id: i64) -> Agent<impl rig::completion::CompletionModel> {
        ...
        let query_tool = super::tools::QuerySystemFilesTool::new(evidence_pool.clone(), app.clone(), evidence_id);
        let extract_tool = super::tools::ExhumeExtractionTool::new(evidence_pool.clone(), app.clone(), evidence_id);
        let analyze_image_tool = super::tools::AnalyzeImageTool::new(evidence_pool.clone(), app.clone(), evidence_id, config.api_key.clone());
        let query_parsed_artifacts_tool = super::tools::QueryParsedArtifactsTool::new(evidence_pool.clone(), app.clone(), evidence_id);
        let query_sqlite_file_tool = super::tools::QuerySqliteFileTool::new(evidence_pool.clone(), app.clone(), evidence_id);
        ...
        client
            ...
            .preamble(
                ...
                Use the `query_sqlite_file` tool to expressly extract and query an explicitly requested target SQLite database (.db or .sqlite) file from the disk image, using the file's `identifier` and `partition_id`. Example: 'SELECT name FROM sqlite_master WHERE type=\"table\"'.
